...
module.exports = class Client {
  ...
  getWorkerUrl(ressources = "", params = "") {
    const url = `${
      ...
      process.env.ZENATON_WORKER_PORT
        ? process.env.ZENATON_WORKER_PORT
        : DEFAULT_WORKER_PORT
    }/api/${WORKER_API_VERSION}/${ressources}?`;
  ...
  getWebsiteUrl(ressources = "", params = "") {
    const url = `${
      process.env.ZENATON_API_URL
        ? process.env.ZENATON_API_URL
        : ZENATON_API_URL
    }/${ressources}?${API_TOKEN}=${this.apiToken}&`;
  ...
  findWorkflow(workflowName, customId) {
    const params = `${ATTR_ID}=${customId}&${ATTR_NAME}=${workflowName}&${ATTR_PROG}=${PROG}`;

    return http
      .get(this.getInstanceWebsiteUrl(params))
      .then((body) =>
